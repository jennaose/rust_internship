...
#[tokio::main] //sets up the asynchronous runtime enviroment
async fn main() {
    ...
    let fetch_tasks: Vec<_> = websites.iter().map(|&url| {
        ...
        task::spawn(async move { //spawn runs through each task concurrently
            let result = fetch_url(client, url.to_string()).await;
            (url.to_string(), result)
        })
    }).collect();

    let mut results = HashMap::new();//to store results
    for task in fetch_tasks {
        match task.await {
            ...
            Ok((url, Err(err))) => {
                eprintln!("Error fetching {}: {}", url, err); //error fetching content
            }
            Err(err) => {
                eprintln!("Task failed: {}", err); //task failed
            }
        }
    }
